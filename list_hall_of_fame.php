...
<?php $res=mysql_query($platoonsql);
while ($platoon=mysql_fetch_array($res)) { ?>
	<a href="index.php?url=list_hall_of_fame.php&platoon=<?php echo $platoon['id']; ?>"><?php echo $platoon['name_long']; ?> (<?php echo $platoon['name_short']; ?>)</a>
<?php } ?>
</div>
...
<?php
	$characterres=mysql_query($glorytopsql);
		$characterarray = characterlisting($characterres, "glory");
		$i = 0;
	...
	<TR>
		...
		<TD><?php if ($admin || $gm || $_SESSION['user_id']==$character['userid']) { ?><a href="index.php?url=modify_character.php&character_id=<?php echo $character['cid'];?>"> <?php } ?><?php echo $character['cfor'] . " " . $character['clast'];?></a></TD>	
...
<?php
	$characterres=mysql_query($deadcharactersql);
		$characterarray = characterlisting($characterres, "dead");
		foreach ($characterarray as $character) { ?>
	<TR>
		<TD><?php echo $character['missions'];?></TD>
		<TD><?php echo $character['rank_short'];?></TD>
...
<?php 	$lastmissionsql="SELECT DATE_FORMAT(date,'%y-%m-%d') as date,mission_name_short FROM {$_SESSION['table_prefix']}mission_names LEFT JOIN {$_SESSION['table_prefix']}missions as m on m.mission_id = {$_SESSION['table_prefix']}mission_names.id WHERE character_id = '{$character['cid']}' ORDER BY date DESC LIMIT 1";
...
<?php $characterres=mysql_query($retiredcharactersql);
		$characterarray = characterlisting($characterres,"retired");
		foreach ($characterarray as $character) { ?>
	<TR>
		<TD><?php echo $character['missions'];?></TD>
		<TD><?php echo $character['rank_short'];?></TD>
