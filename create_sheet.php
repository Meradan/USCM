...
//$logoimage=pdf_open_image_file($pdf,"png","./resources/logoimage.png","",0);
//pdf_place_image($pdf,$logoimage,30,700,0.9);
//$logoimage=pdf_open_image_file($pdf,"png","./resources/logoimage_sized.png","",0);
//pdf_place_image($pdf,$logoimage,50,700,0.7);
//PDF_fit_image($pdf, "./resources/logoimage.png", 50, 700, "/boxsize 50 700");
...
//      pdf_fill_stroke($pdf);
$pdf->Rect($xpos, 484, $width, $height, 'DF', array('all' => array('width' => 1, 'cap' => 'round', 'join' => 'round', 'color' => array(0, 0, 0))), array(190, 190, 190));
   }
   $xpos += $width;
}
...
for ($i=1;$i<=10;$i++) {
   if ($i <= $white ) {
       $pdf->Rect($xpos, 496, $width, $height, 'DF', array('all' => array('width' => 1, 'cap' => 'round', 'join' => 'round', 'color' => array(0, 0, 0))), array(255, 255, 255));
    } else {
        $pdf->Rect($xpos, 496, $width, $height, 'DF', array('all' => array('width' => 1, 'cap' => 'round', 'join' => 'round', 'color' => array(0, 0, 0))), array(190, 190, 190));
    }
    $xpos += $width;
}
pdf_setcolor($pdf,"fill","gray",0,0,0,0);
