...
include ("controllers/rank_controller.php");
...
function getPlatoons() {
  ...
  $sql = "SELECT id, name_short, name_long
                FROM " . $tablePrefix . "platoon_names";
  $stmt = $db->prepare($sql);
  $stmt->execute();
  while ( $row = $stmt->fetch(PDO::FETCH_ASSOC) ) {
    ...
    $certsql = "SELECT certificate_id FROM " . $tablePrefix . "platoon_certificates
        WHERE platoon_id = :platoonid";
    $stmt_cert = $db->prepare($certsql);
